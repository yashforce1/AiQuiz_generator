...
import { Brain, LayoutDashboard, ClipboardList, MessageSquare, LogOut, Menu, X } from 'lucide-react'
import { useApp } from '../../context/AppContext.jsx'

const navItems = [
  { to: '/student', icon: <LayoutDashboard size={18} />, label: 'Dashboard', end: true },
  { to: '/student/my-tests', icon: <ClipboardList size={18} />, label: 'My Tests' },
  { to: '/student/ai-tutor', icon: <MessageSquare size={18} />, label: 'AI Tutor' },
]
        ...
        <div style={{ padding: '16px 12px', borderTop: '1px solid var(--border)' }}>
          {sidebarOpen && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 'var(--radius-md)', background: 'var(--bg-elevated)', marginBottom: 8 }}>
              <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'linear-gradient(135deg, #10b981, #059669)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, flexShrink: 0 }}>
                {user?.avatar}
              </div>
              <div style={{ overflow: 'hidden' }}>
                <p style={{ fontSize: 13, fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{user?.name}</p>
                <p style={{ fontSize: 11, color: 'var(--text-muted)' }}>{user?.stream} • {user?.class}</p>
              </div>
            </div>
          )}
